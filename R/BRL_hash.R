#' BRL Hash: Bipartite Record Linkage
#'
...
#' \enumerate{
#'   \item \code{\link{BRL_hash_update_m_u}} to update m and u for BRL hash
#'   \item \code{\link{BRL_hash_sample_pair}} to sample a link for a given record, indicating
#'   whether it is linked to one of the existing records (\code{1..n1}) or remains unlinked (\code{0})
#'   \item \code{\link{BRL_hash_compute_unique_weights}} to calculate the weight for each pattern based
#'   on the current \eqn{m} and \eqn{u} parameters.
#' }
#'
#' @param hash_data A list containing the hashed comparison data, with elements:
#'   \describe{
...
#' @param seed An integer for random number generator (RNG) seeding.
...
BRL_hash <- function(hash_data,
  ...
  #-----------------------------------------------------------------
  # 1) Set seed and extract data from 'hash_data'
  #-----------------------------------------------------------------
  set.seed(seed)
  ...
  pair_to_pattern <- hash_data$pair_to_pattern
    ...
    # Define a vector 0..P for sampling pattern indices (0 => unlinked).
  candidates_P <- 0:P
  ...
  m <- u <- rep(0, length(field_marker)) # m & u are updated each iteration, initialize them to zeros
  ...
  for (s in seq_len(S)) {
    ...
    # --- 4a) Update m and u using helper function 'BRL_hash_update_m_u' ---
    res_mu <- BRL_hash_update_m_u(unique_patterns, pattern_counts, matches,
                         field_marker, m_prior, u_prior)
    m <- res_mu$m
    u <- res_mu$u

    # --- 4b) Compute weights for each pattern using 'BRL_hash_compute_unique_weights' ---
    unique_weights <- BRL_hash_compute_unique_weights(m, u, unique_patterns)
    ...
    for (j in seq_len(n2)) {
      ...
      # Use helper function 'BRL_hash_sample_pair' to do mode-specific sampling
      out <- BRL_hash_sample_pair(
        j = j,
        Z = Z,
        Z_inv = Z_inv,
        L = L,
