...
hash_comparisons_original <- function(cd,
  ...
  hash_vals <- purrr::imap(cd[[4]], ~hash_field_original(.x, .y, Lf_vec)) %>%
    unlist()
  ...
  if(all_patterns == TRUE){
    ...
    unique_patterns <- possible_patterns_ohe_original(levels)
